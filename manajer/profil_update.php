...
if($filename == ""){
	...
	if(!in_array($ext,$allowed) ) {
		...
		if($_POST['password']==""){
			mysqli_query($koneksi,"update user set alamat='$alamat', kontak='$kontak', email='$email', username='$username', foto='$foto' where id=$id");
			header("location:index.php?alert=profil");
		}else{
			mysqli_query($koneksi,"update user set alamat='$alamat', kontak='$kontak', email='$email', username='$username', password='$password', foto='$foto' where id=$id");
			header("location:index.php?alert=profil");
		}
	}
}
